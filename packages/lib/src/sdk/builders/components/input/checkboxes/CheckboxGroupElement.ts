/**
 * @summary Single choice in a checkbox group.
 * @description Nested checkbox option payload (not a Discord component type). Holds a developer `value`, user-facing `label`, and optional description and default. Place elements on {@link CheckboxGroup}.
 * @example
 * ```ts
 * const option: CheckboxGroupElement = new CheckboxGroupElement()
 *     .setValue("march-4")
 *     .setLabel("March 4th")
 * ```
 */
class CheckboxGroupElement {
     ...
     * @description Discord caps checkbox option `value`, `label`, and `description` at 100 characters.
     */
     ...
     * @description Text shown next to the checkbox. Required when serializing.
     */
    ...
    /**
     * @summary Creates a checkbox group option.
     * @description Starts with no value or label. Set both with {@link CheckboxGroupElement.setValue} and {@link CheckboxGroupElement.setLabel} before sending.
     */
    public constructor() {}
    ...
    public setValue(value: string): this {
        this.value = CheckboxGroupElement.assertText("value", value)
        return this
    }
    ...
    public setLabel(label: string): this {
        this.label = CheckboxGroupElement.assertText("label", label)
        return this
    }
    ...
    public setDescription(description: string): this {
        this.description = CheckboxGroupElement.assertText("description", description)
        return this
    }

    /**
     * @summary Gets whether this option is selected by default.
    ...
    /**
     * @summary Sets whether this option is selected by default.
     ...
     * @description Builds a Discord checkbox group option with required `value` and `label`, plus optional `description` and `default`.
     * @returns The JSON object representation of the option.
     */
    public toJSON(): Record<string, unknown> {
        const payload: Record<string, unknown> = {
            value: CheckboxGroupElement.assertText("value", this.value),
            label: CheckboxGroupElement.assertText("label", this.label),
        }
    ...
    private static assertText(field: string, value: string | undefined): string {
        if (typeof value !== "string") {
            throw new TypeError(`Checkbox group option ${field} must be a string.`)
        }
        if (value.length < 1 || value.length > CheckboxGroupElement.MAX_TEXT_LENGTH) {
            throw new RangeError(`Checkbox group option ${field} must be between 1 and 100 characters.`)
        }
        return value
    }
}
